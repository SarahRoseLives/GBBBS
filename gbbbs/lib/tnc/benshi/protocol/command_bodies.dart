...
// Abstract base class for all reply bodies to standardize status checking.
...
// --- HT_SEND_DATA ---
class HTSendDataBody extends MessageBody {
  final TncDataFragment tncDataFragment;
  HTSendDataBody({required this.tncDataFragment});

  @override
  Uint8List toBytes() => tncDataFragment.toBytes();
}

class HTSendDataReplyBody extends ReplyBody {
  HTSendDataReplyBody({required super.replyStatus});
  factory HTSendDataReplyBody.fromBytes(Uint8List bytes) {
    return HTSendDataReplyBody(replyStatus: ReplyStatus.fromInt(bytes[0]));
  }
  @override
  Uint8List toBytes() => throw UnimplementedError("Reply bodies are not intended to be converted to bytes.");
}
